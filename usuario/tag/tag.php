...
if( isset($_SESSION['rss2']) OR isset($_SESSION['importante']) OR isset($_SESSION['protegido']) ){
...
if( isset($_POST['tags']) ){
	...
	if( isset($_GET['t']) ){
		$tag = filter_var($_GET['t'], FILTER_SANITIZE_STRING);
		unset($_GET['t']);
	} else {
		$tag = filter_var($_GET['t'], FILTER_SANITIZE_STRING);
		unset($_GET['t']);
...
$enQueColumna = 'id';
$queBuscar = $tag;
$tagNombre = mysql02($dondeBuscar,$tabla,$enQueColumna,$queBuscar);

$campos = 'id, titulo, fecha, enlace, visitas, protegido, importante';
$tabla1 = 'feeds';
$columna1 = 'usuarioId';
...
echo "<h2>".ucwords($tagNombre)."</h2>\n";
echo "<table><tr><td>";
echo $nroRss." art. sin leer | </td>";

//----------------------------------------------------------------------
// PAGINACION
$page_name = "?w=tag&t=".$tagId;
$t = mysql_query("SELECT count(*) cnt FROM feeds where usuarioId = '$usuarioId' AND eliminado = '0' AND tagId = '$tagId' ");
...
$q = mysql_query("SELECT id, titulo, fecha, enlace, tagId, visitas, protegido, importante FROM feeds
WHERE usuarioId = '$usuarioId' AND eliminado = '0' AND tagId = '$tagId' ORDER BY fecha DESC LIMIT $set_limit, $limit ");
...
_EMAIL;
		...
		// eliminar
		$eliminar = <<<_ELIMINAR
		<td><a href="?w=eliminar3&i=$value[id]&r=$rssId&o=protegido"><img src="imagenes/eliminar.png" width="40" height="40" alt="Eliminar"/></a></td>
_ELIMINAR;
		echo $eliminar;
		...
		<td><a href="?w=protegido2&i=$value[id]&r=$rssId&o=protegido&p=$value[protegido]"><img src="imagenes/$imagenP.png" width="40" height="40" alt="Protegido"/></a></td>\n
_PROTEGIDO;
		echo $protegido;
		...
		<td><a href="?w=importante2&i=$value[id]&r=$rssId&o=protegido&im=$value[importante]"><img src="imagenes/$imagen.png" width="40" height="40" alt="Importante"/></a></td>
_IMPORTANTE;
		echo $importante;
